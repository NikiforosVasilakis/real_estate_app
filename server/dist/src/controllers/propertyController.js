...
const getProperties = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let whereCondition = [];
        if (favoritesID) {
            const favoritesIdsArray = favoritesID.split(",").map(Number);
            whereCondition.push(client_1.Prisma.sql `p.id IN (${client_1.Prisma.join(favoritesIdsArray)})`);
        }
        if (priceMin) {
        ...
        if (propertyType && propertyType !== "any") {
            whereCondition.push(client_1.Prisma.sql `p.propertyType = ${propertyType}:: "PropertyType"`);
        }
        if (squareFeetMin) {
            whereCondition.push(client_1.Prisma.sql `p."squareFeetMin" >= ${Number({ squareFeetMin })}`);
        }
        if (squareFeetMax) {
            whereCondition.push(client_1.Prisma.sql `p."squareFeetMax" <= ${Number({ squareFeetMax })}`);
        }
        if (amenities && amenities !== "any") {
            const amenitiesAray = amenities.split(",").map(Number);
            whereCondition.push(client_1.Prisma.sql `p.amenities @> ${amenitiesAray}`);
        }
        if (availableFrom && availableFrom !== "any") {
            ...
            if (availableFromDate) {
                ...
                if (!isNaN(date.getTime())) {
                    whereCondition.push(client_1.Prisma.sql `EXISTS(SELECT 1 FROM "lease" l WHERE l."propertyID" = p.id AND l."startDate" <= ${date.toISOString()})`);
                }
            }
        }
        if (latitude && longitude) {
            ...
            whereCondition.push(client_1.Prisma.sql `ST_DWithin(l.coordinates::geometry,ST_SetSRID(ST_MakePoint(${lon},${lat}),4326),${degrees})`);
        }
        const completeQuery = client_1.Prisma.sql `SELECT p.* json_build_object(
                'id', l.id, 
                'address', l.address
                'city', l.city,
                'state', l.state,   
            ...
            ${whereCondition.length > 0 ? client_1.Prisma.sql `WHERE ${client_1.Prisma.join(whereCondition, " AND ")}` : client_1.Prisma.empty}`;
...
const createProperty = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const photoUrls = yield Promise.all(files.map((file) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            const uploadResault = yield new lib_storage_1.Upload({
                client: s3Client,
                params: uploadParams,
            }).done();
        ...
        const geocodingResponce = yield axios_1.default.get(geocodingUrl, {
            headers: {
                "User-Agent": "realestateapp"
            }
        });
        const [longitude, latitude] = ((_a = geocodingResponce.data[0]) === null || _a === void 0 ? void 0 : _a.lon) && ((_b = geocodingResponce.data[0]) === null || _b === void 0 ? void 0 : _b.lat) ?
            [
                parseFloat((_c = geocodingResponce.data[0]) === null || _c === void 0 ? void 0 : _c.lon),
                parseFloat((_d = geocodingResponce.data[0]) === null || _d === void 0 ? void 0 : _d.lat),
            ] : [0, 0];
